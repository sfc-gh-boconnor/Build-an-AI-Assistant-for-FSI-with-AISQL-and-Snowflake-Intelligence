...
CREATE STAGE IF NOT EXISTS ACCELERATE_AI_IN_FSI.CORTEX_ANALYST.cortex_analyst
...
-- =====================================================
-- CREATE STUB TABLES FOR SEMANTIC VIEWS
-- =====================================================
-- These tables are required for semantic view creation
-- They will be dropped after semantic views are created
-- Users will create the real tables with data using notebooks
-- =====================================================

-- Stub for INFOGRAPHICS_FOR_SEARCH (created by Notebook 4)
CREATE OR REPLACE TABLE ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.INFOGRAPHICS_FOR_SEARCH (
    ...
    BRANDING VARCHAR,
    TEXT VARCHAR,
    CUSTOMERS_1M_PLUS VARCHAR,
    FREE_CASH_FLOW VARCHAR,
    GROSS_MARGIN VARCHAR,
    NET_REVENUE_RETENTION VARCHAR,
...
-- Stub for VW_FINANCIAL_SUMMARY (created by Notebook 1)
CREATE OR REPLACE VIEW ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_FINANCIAL_SUMMARY AS
SELECT 
    'stub' AS COMPANY,
    'stub' AS TICKER,
    'stub' AS PERIOD,
    'stub' AS RELATIVE_PATH,
    'stub' AS TOTAL_REVENUE_Q2_2024,
    'stub' AS TOTAL_REVENUE_Q2_2025,
    'stub' AS YOY_GROWTH,
    'stub' AS GROSS_MARGIN,
    'stub' AS OPERATING_MARGIN,
    'stub' AS FREE_CASH_FLOW,
    'stub' AS NRR,
    'stub' AS TOTAL_CUSTOMERS
WHERE 1=0;

-- Stub for VW_INCOME_STATEMENT (created by Notebook 1)
CREATE OR REPLACE VIEW ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_INCOME_STATEMENT AS
SELECT 
    'stub' AS COMPANY,
    'stub' AS TICKER,
    'stub' AS PERIOD,
...
-- Stub for VW_KPI_METRICS (created by Notebook 1)
CREATE OR REPLACE VIEW ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_KPI_METRICS AS
SELECT 
    'stub' AS COMPANY,
    'stub' AS TICKER,
    'stub' AS PERIOD,
...
-- Stub for ANALYST_REPORTS (created by Notebook 1)
CREATE OR REPLACE TABLE ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.ANALYST_REPORTS (
    ...
    PRICE_TARGET FLOAT,
    GROWTH FLOAT,
    NAME_OF_REPORT_PROVIDER VARCHAR,
...
-- Stub for SENTIMENT_ANALYSIS (created by Notebook 2)
...
-- Stub for TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT (created by Notebook 2)
...
-- Stub for TRANSCRIPTS_BY_MINUTE (created by Notebook 2)
...
-- Stub for SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH (view created by script 02, but needs stub for semantic view)
CREATE OR REPLACE VIEW ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH AS
SELECT 
    'stub' AS PRIMARY_TICKER,
    CURRENT_TIMESTAMP() AS EVENT_TIMESTAMP,
    'stub' AS EVENT_TYPE,
    CURRENT_TIMESTAMP() AS CREATED_AT,
    0 AS SENTIMENT_SCORE,
    0 AS UNIQUE_ANALYST_COUNT,
    'stub' AS SENTIMENT_REASON,
    'stub' AS FULL_TRANSCRIPT_TEXT,
    0 AS TRANSCRIPT_LENGTH
WHERE 1=0;

CREATE OR REPLACE SEMANTIC VIEW ACCELERATE_AI_IN_FSI.CORTEX_ANALYST.COMPANY_DATA_8_CORE_FEATURED_TICKERS
	TABLES (
		DEFAULT_SCHEMA.INFOGRAPHICS_FOR_SEARCH 
			PRIMARY KEY (TICKER) 
...
CREATE OR REPLACE SEMANTIC VIEW ACCELERATE_AI_IN_FSI.CORTEX_ANALYST.SNOWFLAKE_ANALYSTS_VIEW
	TABLES (
		DEFAULT_SCHEMA.ANALYST_REPORTS COMMENT='This table stores information about analyst reports, including the report''s relative path, URL, rating, date, close price, price target, growth, report provider, document type, document, summary, full text, and URL.',
		DEFAULT_SCHEMA.SENTIMENT_ANALYSIS COMMENT='This table stores the results of sentiment analysis on text data related to products or services, capturing the overall sentiment and specific aspects such AS cost, innovation, productivity, competitiveness, and consumption, with each aspect scored on a numerical scale.',
		DEFAULT_SCHEMA.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT COMMENT='This table stores transcribed earnings call data with sentiment analysis, capturing the audio duration, speaker information, and text segments along with their corresponding sentiment scores, allowing for analysis of tone and emotional content in corporate earnings calls.',
		DEFAULT_SCHEMA.TRANSCRIPTS_BY_MINUTE COMMENT='This table stores transcripts of audio or video recordings, broken down into minute-by-minute segments, with additional metadata such AS speaker information, sentiment analysis, and the actual text spoken during each minute.',
		DEFAULT_SCHEMA.STOCK_PRICES COMMENT='This table stores historical stock price data, including daily high and low prices, trading volume, and pre- and post-market prices, for various stocks across different exchanges, categorized by asset class, with data aggregated by date, month, and year.'
	)
	FACTS (
...
-- =====================================================
-- DROP STUB TABLES/VIEWS
-- =====================================================
-- These were only needed for semantic view creation
-- Users will create the real tables with data using notebooks
-- =====================================================

DROP TABLE IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.INFOGRAPHICS_FOR_SEARCH;
DROP VIEW IF EXISTS ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_FINANCIAL_SUMMARY;
DROP VIEW IF EXISTS ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_INCOME_STATEMENT;
DROP VIEW IF EXISTS ACCELERATE_AI_IN_FSI.DOCUMENT_AI.VW_KPI_METRICS;
DROP TABLE IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.ANALYST_REPORTS;
DROP TABLE IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.SENTIMENT_ANALYSIS;
DROP TABLE IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.TRANSCRIBED_EARNINGS_CALLS_WITH_SENTIMENT;
DROP TABLE IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.TRANSCRIPTS_BY_MINUTE;
DROP VIEW IF EXISTS ACCELERATE_AI_IN_FSI.DEFAULT_SCHEMA.SENTIMENT_WITH_TRANSCRIPTS_FOR_SEARCH;

SELECT '✅ Cortex Analyst deployment complete!' AS status,
       '📊 2 Semantic Views created' AS semantic_views,
       '🤖 1 Agent created (One Ticker)' AS agent,
       '📓 Run notebooks to populate tables with real data!' AS next_step;